function [drawRec] = update_rectangle(corners,drawRec, i,len_obj,ngridpoints,color,curvature);
...
drawRect.rect(i)=rectangle('Position',[corners(i,1)-(len_obj)/2,corners(i,2)-...
    (len_obj)/2,len_obj,len_obj],'Curvature',[curvature,curvature],'FaceColor',color);

%draw the side rectangles if needed
if corners(i,1)+len_obj/2>ngridpoints %overlaps right edge
    drawRec.rect_hor(i)=rectangle('Position',[corners(i,1)-ngridpoints-...
        (len_obj)/2,corners(i,2)-...
        (len_obj)/2,len_obj,len_obj],'Curvature',[curvature,curvature],'FaceColor',color);
elseif corners(i,1)-len_obj/2<1; %left edge
    drawRec.rect_hor(i)=rectangle('Position',[corners(i,1)+ngridpoints-(len_obj)/2,corners(i,2)-...
        (len_obj)/2,len_obj,len_obj],'Curvature',[curvature,curvature],'FaceColor',color);
end
if corners(i,2)+len_obj/2>ngridpoints %top edge
    drawRec.rect_ver(i)=rectangle('Position',[corners(i,1)-...
        (len_obj)/2,corners(i,2)-ngridpoints-(len_obj)/2,...
        len_obj,len_obj],'Curvature',[curvature,curvature],'FaceColor',color);
elseif corners(i,2)-len_obj/2<1; %bottom edge
    drawRec.rect_ver(i)=rectangle('Position',[corners(i,1)-...
        (len_obj)/2,corners(i,2)+ngridpoints-(len_obj)/2,...
        len_obj,len_obj],'Curvature',[curvature,curvature],'FaceColor',color);
end
